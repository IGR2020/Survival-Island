...
def get_land_from_image(
    image_path, water_land_check=True, create_gateway=False, gateway_link=None
    ...
    tree_noise = perlin_noise.PerlinNoise()
    for x in range(width):
        land.append([])
        for y in range(height):
            if image.get_at((x, y)) == (239, 228, 176, 255):
                if randint(0, 1) == 0:
                    land[-1].append(
                        Block(x * blockSize, y * blockSize, blockSize, "Sand")
                    )
                else:
                    land[-1].append(
                        Block(x * blockSize, y * blockSize, blockSize, "Flat Sand")
                    )
                if randint(0, 40) == 0:
                    structures.append(
                        Structure(
                            x * blockSize - treeSize / 3,
                            y * blockSize - treeSize / 3,
                            choice(("Palm Tree1", "Palm Tree2", "Palm Tree3")),
                            [Item("Wood", randint(1, 6))],
                        )
                    )
            elif image.get_at((x, y)) == (0, 0, 0, 255):
                if randint(0, 1) == 0:
                    land[-1].append(
                        Block(
                            x * blockSize, y * blockSize, blockSize, "Flat Dead Grass"
                        )
                    )
                else:
                    land[-1].append(
                        Block(x * blockSize, y * blockSize, blockSize, "Dead Grass")
                    )
                    
                structure_added = get_dead_tree_by_noise(x, y, tree_noise)
                ...
                structure_added = get_grass_tree_by_noise(x, y, tree_noise)
                if structure_added is not None:
                    structures.append(structure_added)
                ...
                land[-1].append(
                    Block(x * blockSize, y * blockSize, blockSize, "Gateway Point")
                )
                structures.append(
                    Gateway(
                        x * blockSize,
                        ...
                        "Gateway",
                        health=64,
                        land_link=gateway_link,
                    )
                )
            elif image.get_at((x, y)) == (181, 230, 29, 255):
                land[-1].append(
                    Block(x * blockSize, y * blockSize, blockSize, "Spawn Point")
                )
                ...
                for reg_x in range(x - 1, x + 2):
                    for reg_y in range(y - 1, y + 2):
                        try:
                            ...
                            if (y == height - 1 or x == width - 1) and randint(
                                0, 4
                            ) == 0:
                                land[-1].append(
                                    Block(
                ...
                if (y == 0 or x == 0) and randint(0, 4) == 0:
                    land[-1].append(
                        Block(
                            x * blockSize, y * blockSize, blockSize, "Calm Deep Water"
                        )
                    )
                    continue
                land[-1].append(get_water_type_by_noise(x, y, sea_noise))
...
def get_dead_tree_by_noise(x, y, tree_noise):
    if abs(tree_noise((x * treeDensityChaos, y * treeDensityChaos))) > 0.2:
        if randint(0, 1):
            return Structure(
                x * blockSize - treeSize / 3,
                y * blockSize - treeSize / 4,
                "Dead Tree1",
                [Item("Wood", randint(1, 5))],
            )
    elif abs(tree_noise((x * treeDensityChaos, y * treeDensityChaos))) > 0.1:
        if randint(0, 4) == 0:
            return Structure(
            x * blockSize - treeSize / 3,
            y * blockSize - treeSize / 4,
...
def get_grass_tree_by_noise(x, y, tree_noise):
    if abs(tree_noise((x * treeDensityChaos, y * treeDensityChaos))) > 0.4:
        if randint(0, 2) == 0:
            return Structure(
                x * blockSize - treeSize / 3,
                y * blockSize - treeSize / 4,
                "Tree1",
                [Item("Wood", randint(2, 9))],
            )
    elif abs(tree_noise((x * treeDensityChaos, y * treeDensityChaos))) > 0.3:
        if randint(0, 7) == 0:
            return Structure(
                x * blockSize - treeSize / 3,
                y * blockSize - treeSize / 4,
    ...
    elif abs(tree_noise((x * treeDensityChaos, y * treeDensityChaos))) > 0.2:
        if randint(0, 10) == 0:
            return Structure(
                x * blockSize - treeSize / 3,
                y * blockSize - treeSize / 4,
...
def get_world_from_directory(dir_path, water_land_check=True, create_land_links=False):
    ...
    for i, path in enumerate(paths):
        land, spawn, stuctures, gateway_point, spawners = get_land_from_image(
            join(dir_path, path), water_land_check, create_land_links, i - 1
        )
        world.append(
            {
                "land": land,
